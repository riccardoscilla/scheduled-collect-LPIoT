...
PROCESS_THREAD(app_process, ev, data) 
{
  ...
  if(linkaddr_cmp(&sink, &linkaddr_node_addr)) {
    printf("App-collect: I am sink %02x:%02x\n",
    ...
    printf("App-collect: I am a normal node %02x:%02x\n",
      linkaddr_node_addr.u8[0], linkaddr_node_addr.u8[1]);
    ...
    while(1) {
      ...
      printf("App-collect: Send seqn %d\n", msg.seqn);
...
recv_cb(const linkaddr_t *originator, uint8_t seqno, uint8_t hops)
{
  ...
  if (packetbuf_datalen() != sizeof(msg)) {
    printf("App-collect: wrong length: %d\n", packetbuf_datalen());
    return;
  }
  memcpy(&msg, packetbuf_dataptr(), sizeof(msg));
  printf("App-collect: Recv from %02x:%02x seqn %d hops %d\n",
