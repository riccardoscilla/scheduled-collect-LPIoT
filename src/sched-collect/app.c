...
PROCESS_THREAD(app_process, ev, data)
{
  ...
  // Test with a char message
  // static test_msg_t msg = {.str= "Hello!"};
  ...
  if(linkaddr_cmp(&sink, &linkaddr_node_addr)) {
    ...
    while(1) {
      ...
      if (ret != 0)
        ...
        // Test with a char message
        // printf("App: packet with string %s could not be scheduled.\n", msg.str);
      msg.seqn++;
      /* Reset timer */
      PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&et));
      etimer_reset(&et);
...
recv_cb(const linkaddr_t *originator, uint8_t hops)
{
  ...
  // Test with a char message
  // printf("App: Recv from %02x:%02x message %s hops %d\n",
  //   originator->u8[0], originator->u8[1], msg.str,  hops);
}
/*---------------------------------------------------------------------------*/
