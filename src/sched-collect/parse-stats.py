...
def parse_file(log_file, testbed=False):
	...
	print("Logfile: {}".format(log_file))
	if not testbed:
		print('Cooja simulation')
	else:
		print('Testbed experiment')
	...
	frecv_name = os.path.join(fpath, "{}-recv.csv".format(fname_common))
	fsent_name = os.path.join(fpath, "{}-sent.csv".format(fname_common))
	fenergest_name = os.path.join(fpath, "{}-energest.csv".format(fname_common))
